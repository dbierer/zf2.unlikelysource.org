...
class UserController extends AbstractActionController
{
    ...
    public function loginAction()
    {
        ...
        if (!$request->isPost()) {
        	...
        	$viewModel->setTemplate($options->getUserLoginWidgetViewTemplate());
            return $viewModel;
        }
    ...
    public function registerAction()
    {
        ...
        // 2013-02-22 DB: check registration key
        $knownKey = $this->getServiceLocator()->get('registration-key');
        $userKey = isset($prg['key']) ? $prg['key'] : 0;
        if ($userKey && in_array($userKey, $knownKey, TRUE)) {
        	$post = $prg;        
	        $user = $service->register($post);
        } else {
        	$this->flashMessenger()->addMessage('Sorry!  Unable to register your account at this time.');
